...
const DesktopNav = props => {
    ...
    return (
        <div className="desktop-nav d-none d-lg-block w-60 text-center position-absolute"
             style={{top: 'calc((100vh / 2) - (386px / 2))', right: '10px'}}>
            <ul className="icons">
                {links.map(link =>
                    <Link to={link.to} className="position-relative" style={{top: '-2px'}} key={link.to}>
                        <li onClick={() => onClickHandler(link.page)}
